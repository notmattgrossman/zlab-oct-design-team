/* autogenerated by Processing revision 1293 on 2026-03-04 */
...
public void setup() {
  ...
  scene = createGraphics(PApplet.parseInt(viewWidth), height);
...
public void draw() {
  ...
  viewWidth = width / 2.0f;
  int newSceneW = max(1, PApplet.parseInt(viewWidth));
...
public int updateState(float mx, float my) {
...
public void renderScene(PGraphics pg, float mx, float my, int fullyGrown) {
...
public void drawHud(PGraphics pg, int fullyGrown) {
...
public void drawCan(PGraphics pg, float x, float y, float scale) {
  pg.pushMatrix();
  pg.translate(x, y);
  pg.scale(scale);
...
class Plant {
  ...
  public void update(boolean isBeingWatered) {
    if (isBeingWatered) {
      if (growth >= 1) {
        watering = false;
        return;
  ...
  public void display(PGraphics pg, float scale) {
    pg.pushMatrix();
    pg.translate(x, y);
    pg.scale(scale);
    float stemStartY = -25;
