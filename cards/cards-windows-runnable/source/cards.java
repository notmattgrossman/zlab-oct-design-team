...
public void draw() {
  ...
  int newSceneH = max(1, PApplet.parseInt(viewHeight));
...
public float cardXForCol(int col) {
  return startX + col * (cardWidth + cardSpacingX);
}

public float cardYForRow(int row) {
  return startY + row * (cardHeight + cardSpacingY);
}

public float cardCenterX(int col) {
  return cardXForCol(col) + cardWidth / 2f;
}

public float cardCenterY(int row) {
